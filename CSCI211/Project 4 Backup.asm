...
######################################
	...
	move $a2, $s1
	jal findAboveBelow
	#push both returned values onto the stack
	#so the print functions can be calle
	sw $v1, 0($sp)
	addi $sp, $sp, -4
	sw $v0, 0($sp)
	addi $sp, $sp, -4
...
#Arguments: $a0 - string to be printed 
...
#Arguments: $a1 - array, $a2 - element count
#Description: the number of values above and below the average in the array
...
findAboveBelow:
	...
	ble $t2, $v0, lessThanAverage
	#increasing values above by 1
	addi $t4, $t4, 1
	
	b wasGreaterThanAverage
