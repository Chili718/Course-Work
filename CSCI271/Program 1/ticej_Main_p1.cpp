...
list<Person> fetchGroupList(list<Person> masterTemp, char nameTemp[name_size], int gSizeTemp, char gPrefTemp)
{
	//because their is no node before the begin() my code did not check the first element in the list this will remove that 
	//to make sure the first node is checked
	bool checked = false;
	...
	//Both bottom and top iterators will iterate once to there starting positions based on the customers
       	bottomIter++;
	//there is no node before begin() like there is with the end of the list so this must be checked 
	//so the iterator will not go before begin() which could be a starting position
	if(topIter != masterTemp.begin())
		topIter--;
	else//if the customer is the begining node it has already been checked
		checked = true;
	...
	for(int i = 0; i < gSizeTemp; i++)
	{
		...
		if(checked == true && bottomIter != masterTemp.end())
		{
		...
		else if(checked == false && bottomIter == masterTemp.end())
		{
                        if(gPrefTemp == 'N')
                        {
				...
				//the list could go until the beginning without finding a valid person so this check is needed
                                 if(topIter != masterTemp.begin() || checked == false)
				 {
					if((*topIter).p_Gender == gPrefTemp)
                                        	(*returnGroupList).push_back(*topIter);
					if(topIter == masterTemp.begin())
						checked = true;
					else
